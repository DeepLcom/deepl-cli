...
function parseRulesArg(input: string): ConfiguredRules {
  ...
  try {
    ...
    throw new ValidationError(
      `--rules JSON is malformed: ${(error as Error).message}`
    );
  }
...
`
    ...
    .addCommand(
      new Command('list')
        ...
        .option(
          '--detailed',
          'Show detailed information including configured rules and custom instructions'
        )
        ...
        .action(
          async (options: {
            ...
            try {
              const styleRulesCommand =
                await createStyleRulesCommand(createDeepLClient);
    ...
    .addCommand(
      new Command('create')
        ...
        .requiredOption(
          '--language <lang>',
          'Target language (e.g. en, de, es)'
        )
        .option(
          '--rules <json>',
          'Configured rules as a JSON object of category → settings (e.g. \'{"punctuation":{"quotation_mark":"use_guillemets"}}\')'
        )
        .addOption(
          new Option('--format <format>', 'Output format')
            .choices(['text', 'json'])
          ...
          async (options: {
            ...
            try {
              ...
              if (options.format === 'json') {
                ...
                Logger.success(
                  chalk.green(`\u2713 Style rule created: ${rule.styleId}`)
                );
    ...
    .addCommand(
      new Command('show')
        ...
        .option(
          '--detailed',
          'Include configured rules and custom instructions'
        )
        .addOption(
          new Option('--format <format>', 'Output format')
            .choices(['text', 'json'])
        ...
        .action(
          async (
            id: string,
            options: { detailed?: boolean; format?: string }
          ) => {
            try {
              const styleRulesCommand =
                await createStyleRulesCommand(createDeepLClient);

              const rule = await styleRulesCommand.show(
                id,
    ...
    .addCommand(
      new Command('update')
        .description(
          'Update a style rule list (rename and/or replace configured rules)'
        )
        .argument('<id>', 'Style rule ID')
        .option('--name <name>', 'New name')
        .option(
          '--rules <json>',
          'Replace configured rules with a JSON object of category → settings'
        )
        .addOption(
          new Option('--format <format>', 'Output format')
            .choices(['text', 'json'])
        ...
        .action(
          async (
            id: string,
            options: { name?: string; rules?: string; format?: string }
          ) => {
            try {
              if (options.name === undefined && options.rules === undefined) {
                throw new ValidationError(
                  'Specify at least one of --name or --rules'
                );
              }
              ...
              let finalRule;
              if (options.rules !== undefined) {
                finalRule = await styleRulesCommand.replaceRules(
                  id,
                  parseRulesArg(options.rules)
                );
              } else {
    ...
    .addCommand(
      new Command('delete')
        ...
        .action(
          async (id: string, options: { yes?: boolean; dryRun?: boolean }) => {
            try {
              if (options.dryRun) {
                Logger.output(
                ...
                Logger.output(
                  chalk.yellow(`[dry-run] Would delete style rule: "${id}"`)
                );
                return;
              }
        ...
        .action(async (styleId: string, options: { format?: string }) => {
          try {
            const styleRulesCommand =
              await createStyleRulesCommand(createDeepLClient);
            const instructions =
              await styleRulesCommand.listInstructions(styleId);
            if (options.format === 'json') {
              Logger.output(
                styleRulesCommand.formatCustomInstructionJson(instructions)
              );
            } else if (options.format === 'table') {
              if (!process.stdout.isTTY) {
                Logger.warn(
                ...
                Logger.output(
                  styleRulesCommand.formatCustomInstructionsList(instructions)
                );
              } else {
                Logger.output(
                  styleRulesCommand.formatCustomInstructionsTable(instructions)
                );
              }
            } else {
              Logger.output(
        ...
        .action(
          async (
            ...
            try {
              const styleRulesCommand =
                await createStyleRulesCommand(createDeepLClient);
              const createOpts: CreateCustomInstructionOptions = {
                label,
                prompt,
              };
              ...
              const instruction = await styleRulesCommand.addInstruction(
                styleId,
                createOpts
              );
              if (options.format === 'json') {
                Logger.output(
                  styleRulesCommand.formatCustomInstructionJson(instruction)
                );
              } else {
                Logger.success(
                  chalk.green(`\u2713 Instruction added: ${instruction.label}`)
                );
                Logger.output(
                  styleRulesCommand.formatCustomInstruction(instruction)
                );
              }
            } catch (error) {
              handleError(error);
        ...
        .action(
          async (
            ...
            try {
              ...
              const instruction = await styleRulesCommand.updateInstruction(
                styleId,
                label,
                updateOpts
              );
              if (options.format === 'json') {
                ...
                Logger.success(
                  chalk.green(
                    `\u2713 Instruction updated: ${instruction.label}`
                  )
                );
                Logger.output(
                  styleRulesCommand.formatCustomInstruction(instruction)
    ...
    .addCommand(
      new Command('remove-instruction')
        ...
        .option(
          '--dry-run',
          'Show what would be removed without performing the operation'
        )
        .action(
          async (
            styleId: string,
            ...
            try {
              if (options.dryRun) {
                Logger.output(
                  chalk.yellow(`[dry-run] No removals will be performed.`)
                );
                Logger.output(
                  chalk.yellow(
                    `[dry-run] Would remove instruction "${label}" from style rule "${styleId}"`
                  )
                );
                return;
              }
              ...
              if (!options.yes) {
                const { confirm } = await import('../../utils/confirm.js');
                const confirmed = await confirm({
                  message: `Remove instruction "${label}" from style rule "${styleId}"?`,
                });
                if (!confirmed) {
                  Logger.info('Aborted.');
                  return;
              ...
              Logger.success(
                chalk.green(`\u2713 Instruction removed: ${label}`)
              );
            } catch (error) {
              handleError(error);
            }
