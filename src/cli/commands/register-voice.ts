...
export function registerVoice(
  ...
  deps: Pick<
    ServiceDeps,
    'getApiKeyAndOptions' | 'createDeepLClient' | 'handleError'
  >
): void {
  ...
  program
    ...
    .requiredOption(
      '-t, --to <languages>',
      'Target language(s), comma-separated, max 5'
    )
    .option(
      '-f, --from <language>',
    ...
    .addOption(
      new Option(
        ...
        'Source language detection mode'
      ).choices(['auto', 'fixed'])
    )
    ...
    .option(
      '--content-type <type>',
      'Audio content type (auto-detected from extension: ogg, opus, webm, mka, flac, mp3, pcm)'
    )
    .option(
      '--chunk-size <bytes>',
      'Audio chunk size in bytes (default: 6400)',
      (v) => parsePositiveInt(v, 'chunk-size', 10_485_760)
    )
    .option(
      '--chunk-interval <ms>',
      'Interval between chunks in ms (default: 200)',
      (v) => parsePositiveInt(v, 'chunk-interval', 60_000)
    )
    .option(
      '--no-stream',
    ...
    .option(
      ...
      (v) => parseNonNegativeInt(v, 'max-reconnect-attempts', 100)
    )
    .optionsGroup('Output:')
    .addOption(
        ...
        try {
          ...
          if (options.glossary) {
            ...
            options.glossary = await glossaryService.resolveGlossaryId(
              ...
              sourceLang
                ? {
                    from: sourceLang as Language,
                    targets: targetLangs as unknown as Language[],
                  }
                : undefined
            );
          }
