...
`
    ...
    .addCommand(
      new Command('create')
        ...
        .argument(
          '<target-lang>',
          'Target language code (comma-separated for multiple, e.g. de,fr,es)'
        )
          ...
          async (
            ...
            try {
              const glossaryCommand =
                await createGlossaryCommand(createDeepLClient);

              const targetLangs = targetLang
                .split(',')
                .map((l) => l.trim()) as Language[];

              const glossary = await glossaryCommand.create(
                name,
              ...
              Logger.output(
                chalk.green('\u2713 Glossary created successfully')
              );
    ...
    .addCommand(
      new Command('entries')
        ...
        .option(
          '--target-lang <lang>',
          'Target language (required for multilingual glossaries)'
        )
        .addOption(
          new Option('--format <format>', 'Output format')
            .choices(['text', 'json'])
        ...
        .action(
          async (
            nameOrId: string,
            options: { targetLang?: string; format?: string }
          ) => {
            try {
              const glossaryCommand =
                await createGlossaryCommand(createDeepLClient);
              ...
              const entries = await glossaryCommand.entries(
                nameOrId,
                targetLang
              );
              if (options.format === 'json') {
            ...
            try {
              if (options.dryRun) {
                const lines = [
                  ...
                  chalk.yellow(
                    `[dry-run] Would delete glossary: "${nameOrId}"`
                  ),
                ];
              ...
              if (!options.yes) {
                ...
                const confirmed = await confirm({
                  message: `Delete glossary "${nameOrId}"?`,
                });
                if (!confirmed) {
                  Logger.info('Aborted.');
                  return;
              ...
              Logger.success(
                chalk.green('\u2713 Glossary deleted successfully')
              );
            } catch (error) {
              handleError(error);
            }
        ...
        .action(
          async (
            ...
            try {
              ...
              const targetLang = options.targetLang as Language | undefined;
              await glossaryCommand.addEntry(
                nameOrId,
                source,
                target,
                targetLang
    ...
    .addCommand(
      new Command('update')
        ...
        .option(
          '--file <path>',
          'TSV/CSV file with entries for dictionary update'
        )
        .action(
          async (
            nameOrId: string,
            options: { name?: string; targetLang?: string; file?: string }
          ) => {
            try {
              ...
              if (options.file && !options.targetLang) {
                throw new ValidationError(
                  '--target-lang is required when using --file'
                );
              }
              ...
              Logger.success(
                chalk.green(
                  `\u2713 Glossary ${parts.join(' and ')} successfully`
                )
              );
            } catch (error) {
              handleError(error);
    ...
    .addCommand(
      new Command('replace-dictionary')
        .description(
          'Replace all entries in a glossary dictionary from a TSV/CSV file'
        )
        .argument('<name-or-id>', 'Glossary name or ID')
        ...
        .action(async (nameOrId: string, targetLang: string, file: string) => {
          try {
            ...
            Logger.success(
              chalk.green(
                `\u2713 Dictionary replaced successfully (${targetLang})`
              )
            );
          } catch (error) {
            handleError(error);
          ...
          async (
            ...
            try {
              if (!options.yes) {
                const { confirm } = await import('../../utils/confirm.js');
                const confirmed = await confirm({
                  message: `Delete dictionary "${targetLang}" from glossary "${nameOrId}"?`,
                });
                if (!confirmed) {
                  Logger.info('Aborted.');
                  return;
              ...
              await glossaryCommand.deleteDictionary(
                nameOrId,
                targetLang as Language
              );
              Logger.success(
                chalk.green(
                  `\u2713 Dictionary deleted successfully (${targetLang})`
                )
              );
            } catch (error) {
              handleError(error);
