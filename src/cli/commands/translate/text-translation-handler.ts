...
export class TextTranslationHandler {
  ...
  async translateText(
    ...
    if (options.customInstruction && options.customInstruction.length > 0) {
      if (options.customInstruction.length > MAX_CUSTOM_INSTRUCTIONS) {
        throw new ValidationError(
          `Maximum ${MAX_CUSTOM_INSTRUCTIONS} custom instructions allowed`
        );
      }
      for (const instruction of options.customInstruction) {
        if (instruction.length > MAX_CUSTOM_INSTRUCTION_CHARS) {
          throw new ValidationError(
            `Custom instruction exceeds ${MAX_CUSTOM_INSTRUCTION_CHARS} character limit (${instruction.length} chars): "${instruction.substring(0, 50)}..."`
          );
        }
      }
    ...
    if (
      options.outlineDetection !== undefined ||
      options.splittingTags ||
      options.nonSplittingTags ||
      options.ignoreTags
    ) {
      if (options.tagHandling !== 'xml') {
        throw new ValidationError(
          'XML tag handling parameters (--outline-detection, --splitting-tags, --non-splitting-tags, --ignore-tags) require --tag-handling xml'
        );
      }
    }
    ...
    if (options.outlineDetection !== undefined) {
      ...
      if (boolValue !== 'true' && boolValue !== 'false') {
        throw new ValidationError(
          '--outline-detection must be "true" or "false"'
        );
      }
    ...
    if (result.detectedSourceLang) {
      Logger.verbose(
        `[verbose] Detected source language: ${result.detectedSourceLang}`
      );
    }
  ...
  private async translateToMultiple(
    text: string,
    options: TranslateOptions
  ): Promise<string> {
    ...
    const { targetLang: _, ...translationOptions } =
