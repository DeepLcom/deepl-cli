...
export function registerTranslate(program: Command, deps: ServiceDeps): void {
  ...
  program
    ...
    .argument(
      '[text]',
      'Text, file path, or directory to translate (or read from stdin)'
    )
    .optionsGroup('Core Options:')
    .option(
      '-t, --to <language>',
      'Target language(s), comma-separated for multiple (uses config default if omitted)'
    )
    .option(
      '-f, --from <language>',
    ...
    .option(
      '-o, --output <path>',
      'Output file path or directory (required for file/directory translation)'
    )
    .optionsGroup('Translation Quality:')
    .addOption(
      new Option(
        '--formality <level>',
        'Formality level (formal/informal are aliases for more/less)'
      ).choices([
        'default',
        'more',
        'less',
    ...
    .option(
      ...
      'Custom instruction for translation (repeatable, max 10, max 300 chars each)',
      (val: string, prev: string[]) => prev.concat([val]),
      [] as string[]
    )
    .option(
      '--style-id <uuid>',
      'Style rule ID for translation (Pro API only, forces quality_optimized model)'
    )
    .addOption(
      new Option('--model-type <type>', 'Model type').choices([
        'quality_optimized',
    ...
    .option(
      '--preserve-code',
      'Preserve code blocks and variables during translation'
    )
    .option(
      '--preserve-formatting',
    ...
    .addOption(
      new Option(
        '--split-sentences <mode>',
        'Sentence splitting (default: on)'
      ).choices(['on', 'off', 'nonewlines'])
    )
    ...
    .option(
      '--enable-minification',
      'Enable document minification (PPTX/DOCX only, reduces file size)'
    )
    .optionsGroup('XML/HTML Options:')
    .addOption(
      new Option('--tag-handling <mode>', 'Tag handling for XML/HTML').choices([
        'xml',
        'html',
      ])
    )
    .addOption(
      new Option(
        ...
        'Tag handling version (v2 improves structure handling, requires --tag-handling)'
      ).choices(['v1', 'v2'])
    )
    .option(
      '--outline-detection <bool>',
      'Control automatic XML structure detection (true/false, default: true, requires --tag-handling xml)'
    )
    .option(
      '--splitting-tags <tags>',
      'Comma-separated XML tags that split sentences (requires --tag-handling xml)'
    )
    .option(
      '--non-splitting-tags <tags>',
      'Comma-separated XML tags that should not be used to split sentences (requires --tag-handling xml)'
    )
    .option(
      '--ignore-tags <tags>',
      'Comma-separated XML tags with content to ignore (requires --tag-handling xml)'
    )
    .optionsGroup('File/Directory Options:')
    .option('--no-recursive', 'Do not recurse into subdirectories')
    .option(
      '--pattern <pattern>',
      'Glob pattern for file filtering (e.g., "*.md")'
    )
    .option(
      '--concurrency <number>',
      'Number of parallel translations (default: 5)',
      parseInt
    )
    .option(
      '--dry-run',
      'Show what would be translated without performing the operation'
    )
    ...
    .option(
      ...
      'Request and display actual billed character count for cost transparency'
    )
    .optionsGroup('Advanced:')
    .option(
      '--no-cache',
      'Bypass cache for this translation (useful for testing)'
    )
    .option(
      '--api-url <url>',
      'Custom API endpoint (e.g., https://api-free.deepl.com/v2 or internal test URLs)'
    )
    .addHelpText(
      'after',
      `
      ...
      async (
        ...
        try {
          if (!options.to) {
            ...
            const targetLangs = configService.getValue<string[]>(
          ...
          if (
            options.glossary &&
            options.glossary.length > MAX_GLOSSARIES_PER_REQUEST
          ) {
            throw new ValidationError(
          ...
          if (options.dryRun) {
            ...
            const isDir =
              !!text && fs.existsSync(text) && fs.statSync(text).isDirectory();
            const isFile =
              !!text &&
              !isDir &&
              fs.existsSync(text) &&
            ...
            if (text) {
              if (isDir) {
                lines.push(
                  chalk.yellow(`[dry-run] Would translate directory: ${text}`)
                );
                lines.push(
                  chalk.yellow(
                    `[dry-run] Output directory: ${options.output ?? '<required>'}`
                  )
                );
                if (options.pattern) {
                  lines.push(
                    chalk.yellow(`[dry-run] File pattern: ${options.pattern}`)
                  );
                }
                lines.push(
                  chalk.yellow(
                    `[dry-run] Recursive: ${options.recursive !== false}`
                  )
                );
              } else if (isFile) {
                lines.push(
                  chalk.yellow(`[dry-run] Would translate file: ${text}`)
                );
                lines.push(
                  chalk.yellow(
                    `[dry-run] Output: ${options.output ?? '<stdout>'}`
                  )
                );
              } else {
                lines.push(
                  chalk.yellow(
                    `[dry-run] Would translate text: "${text.length > 80 ? text.substring(0, 80) + '...' : text}"`
                  )
                );
              }
            } else {
              lines.push(
                chalk.yellow(`[dry-run] Would translate text from stdin`)
              );
            }

            lines.push(
              chalk.yellow(
                `[dry-run] Target language(s): ${targetLangs.join(', ')}`
              )
            );
            if (options.from) {
              lines.push(
                chalk.yellow(`[dry-run] Source language: ${options.from}`)
              );
            }
            if (options.glossary && options.glossary.length > 0) {
              const label =
                options.glossary.length > 1 ? 'Glossaries' : 'Glossary';
              lines.push(
                chalk.yellow(
                  `[dry-run] ${label}: ${options.glossary.join(', ')}`
                )
              );
            }
            if (options.translationMemory) {
              lines.push(
                chalk.yellow(
                  `[dry-run] Translation memory: ${options.translationMemory}`
                )
              );
              lines.push(
                chalk.yellow(
                  `[dry-run] Match threshold: ${options.tmThreshold ?? 75}`
                )
              );
              lines.push(
                chalk.yellow(
                  `[dry-run] Model: quality_optimized (forced by --translation-memory)`
                )
              );
            }
            if (options.formality) {
              lines.push(
                chalk.yellow(`[dry-run] Formality: ${options.formality}`)
              );
            }
          ...
          const translateCommand = await createTranslateCommand(
            deps,
            options.apiUrl
          );
          ...
          if (text !== undefined) {
            result = await translateCommand.translate(
              text,
              options as { to: string } & typeof options
            );
          } else {
            result = await translateCommand.translateFromStdin(
              options as { to: string } & typeof options
            );
          }
