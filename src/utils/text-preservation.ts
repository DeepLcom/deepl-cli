 ...
 *
 * The tokens are generated by a counter, so a source string carrying a literal
 * `__VAR_0__` — which is ordinary text, not a variable — could otherwise be
 * handed the same token as a real variable. Two occurrences would then reach the
 * engine: `restorePlaceholders` uses `replaceAll`, so the literal copy would be
 * rewritten into the preserved value, and `unresolvedPlaceholders` would accept
 * that copy as proof the substituted token had survived, reporting a genuine loss
 * as intact.
 *
