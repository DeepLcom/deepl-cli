...
const ELEMENT_TAG_RE =
  /<(button|a|h[1-6]|p|span|label|th|td|li|option|input|textarea|select|summary|figcaption|legend|caption|dt|dd|title)[\s>/]/g;
...
function resolveRole(segment: string): string {
  ...
  for (let i = 1; i <= parts.length; i++) {
    ...
    if (
      ROLE_MAP[prefix] &&
      suffix.length > 0 &&
      suffix.every((s) => ROLE_MODIFIERS.includes(s))
    ) {
...
export function buildKeyPatterns(functionNames: string[]): RegExp[] {
  return functionNames.map((name) => {
    ...
    if (name.includes('.')) {
      return new RegExp(`\\b${escaped}\\(\\s*['"]([^'"]+)['"]\\s*[,)]`, 'g');
    }
      ...
      while ((match = pattern.exec(line)) !== null) {
        ...
        if (key) {
          matches.push({
            ...
            filePath,
            line: lineIndex + 1,
            surroundingCode: extractSurroundingCode(
              lines,
              lineIndex,
              contextLines
            ),
...
export function synthesizeContext(
  ...
  for (const m of displayed) {
    parts.push(
      `Used as ${m.matchedFunction}() in ${m.filePath}:${m.line}:\n${m.surroundingCode}`
    );
  }
...
export function buildTemplateLiteralPatterns(
  functionNames: string[]
): RegExp[] {
...
export function extractTemplateLiteralMatches(
  ...
  const filteredNames = functionNames.filter(
...
export async function extractAllKeyContexts(
  ...
  const scanPatterns = options.scanPaths
    .map((scanPath) => {
      ...
      if (
        prefixResolved !== normalizedRoot &&
        !prefixResolved.startsWith(normalizedRoot + path.sep)
      ) {
        Logger.warn(
          `Skipping scan_path "${scanPath}": resolves outside project root`
        );
        return null;
      }
      return path.isAbsolute(scanPath)
        ? scanPath
        : path.join(options.rootDir, scanPath);
    })
  ...
  for (const file of files) {
    ...
    const templateMatches = extractTemplateLiteralMatches(
      content,
      relPath,
      functionNames,
      contextLines
  ...
  for (const [key, matches] of allMatches) {
    ...
    const elementType = firstMatch
      ? extractElementType(firstMatch.surroundingCode)
      : null;
    keyContexts.set(key, {
      key,
