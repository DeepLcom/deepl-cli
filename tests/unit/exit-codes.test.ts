...
describe('ExitCode', () => {
  ...
  describe('getExitCodeFromError', () => {
    it.each<[string, string, ExitCode]>([
      ...
      ['not found', 'File not found', ExitCode.InvalidInput],
      ['unsupported', 'Unsupported format', ExitCode.InvalidInput],
      ['invalid (generic)', 'Invalid target language', ExitCode.InvalidInput],
      ['"is required"', 'Target language is required', ExitCode.InvalidInput],
      ['expected', 'Expected 2 columns', ExitCode.InvalidInput],
