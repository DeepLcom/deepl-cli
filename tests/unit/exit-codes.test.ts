...
describe('ExitCode', () => {
  ...
  describe('exitCodeForError', () => {
    it.each<[string, string, ExitCode]>([
      ...
      [
        'connection reset phrase',
        'Connection reset by peer',
        ExitCode.NetworkError,
      ],
      [
        'connection timed out phrase',
      ...
      [
        'not supported for',
        'Formality not supported for EN',
        ExitCode.InvalidInput,
      ],
      [
        'not supported in',
        'Feature not supported in free plan',
        ExitCode.InvalidInput,
      ],
      [
      ...
      [
        'cannot specify both',
        'Cannot specify both --source and --auto-detect',
        ExitCode.InvalidInput,
      ],
    ...
    it.each<[string, string]>([
      [
        'bare "connection" without qualifying phrase',
        'Database connection pool exhausted',
      ],
      ['bare "timeout" without qualifying phrase', 'Lock acquisition timeout'],
      [
        'bare "required" without "is required"',
        'All fields required by the API',
      ],
      ['bare "config" in unrelated context', 'The config value was updated'],
      [
        'ambiguous "Database connection required for config"',
        'Database connection required for config',
      ],
      [
        'ambiguous "timeout waiting for required resource"',
        'timeout waiting for required resource',
      ],
    ...
    ])('should fall back to GeneralError for %s ("%s")', (_, message) => {
    ...
    it('should log verbose warning when classifyByMessage is invoked', () => {
      ...
      expect(verboseSpy).toHaveBeenCalledWith(
        expect.stringContaining('Untyped error')
      );
  ...
  describe('isRetryableError', () => {
    ...
    ])('should mark %s (code %i) as retryable=%s', (_, exitCode, expected) => {
