...
describe('registerConfig', () => {
  ...
  beforeEach(() => {
    ...
    (ConfigCommand as jest.Mock).mockImplementation(
  ...
  describe('config get', () => {
    it('should get and output a config value', async () => {
      ...
      expect(Logger.output).toHaveBeenCalledWith(
        JSON.stringify('some-value', null, 2)
      );
    });
    ...
    it('should call handleError on failure', async () => {
      ...
      expect(handleError).toHaveBeenCalledWith(
        expect.objectContaining({ message: 'get failed' })
      );
    });
  });
  ...
  describe('config set', () => {
    it('should set a config value', async () => {
      ...
      expect(mockConfigCommandInstance.set).toHaveBeenCalledWith(
        'api.usePro',
        'true'
      );
      expect(Logger.success).toHaveBeenCalledWith(
        expect.stringContaining('Set api.usePro = true')
      );
    });
    ...
    it('should mask API key in success message', async () => {
      ...
      expect(Logger.success).toHaveBeenCalledWith(
        expect.stringContaining('supe...-123')
      );
      expect(Logger.success).not.toHaveBeenCalledWith(
        expect.stringContaining('super-secret-key-123')
      );
    });
  ...
  describe('config list', () => {
    ...
    it('should call handleError on failure', async () => {
      mockConfigCommandInstance.list.mockRejectedValue(
  ...
  describe('config reset', () => {
    ...
    it('should call handleError on failure', async () => {
      mockConfigCommandInstance.reset.mockRejectedValue(
