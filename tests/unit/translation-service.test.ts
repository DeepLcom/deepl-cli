...
describe('TranslationService', () => {
  ...
  describe('static preserveCodeBlocks()', () => {
    it('should replace inline code blocks with placeholders', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveCodeBlocks('Use `console.log()` here', map);

      expect(result).toBe('Use __CODE_0__ here');
      expect(map.get('__CODE_0__')).toBe('`console.log()`');
    });

    it('should replace multi-line code blocks with placeholders', () => {
      const map = new Map<string, string>();
      const text = 'Example:\n```\nconst x = 1;\n```\nDone';
      const result = TranslationService.preserveCodeBlocks(text, map);

      expect(result).toBe('Example:\n__CODE_0__\nDone');
      expect(map.get('__CODE_0__')).toBe('```\nconst x = 1;\n```');
    });

    it('should handle multiple code blocks with sequential counters', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveCodeBlocks('`a` and `b`', map);

      expect(result).toBe('__CODE_0__ and __CODE_1__');
      expect(map.size).toBe(2);
    });

    it('should return text unchanged when no code blocks present', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveCodeBlocks('Hello world', map);
  ...
  describe('static preserveVariables()', () => {
    it('should replace ${var} style variables', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveVariables('Hello ${name}', map);

      expect(result).toContain('__VAR_');
      expect(result).not.toContain('${name}');
      expect([...map.values()]).toContain('${name}');
    });
    ...
    it('should replace printf %s and %d variables', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveVariables('Hello %s, age %d', map);

      expect(result).not.toContain('%s');
      expect(result).not.toContain('%d');
      expect([...map.values()]).toContain('%s');
      expect([...map.values()]).toContain('%d');
    });

    it('should handle text with no variables', () => {
      const map = new Map<string, string>();
      const result = TranslationService.preserveVariables('Hello world', map);
  ...
  describe('static restorePlaceholders()', () => {
    it('should restore all placeholders from the map', () => {
      const map = new Map<string, string>();
      map.set('__CODE_0__', '`console.log()`');
      map.set('__VAR_0__', '{name}');

      const result = TranslationService.restorePlaceholders(
        'Usa __CODE_0__ para __VAR_0__',
        map
      );

      expect(result).toBe('Usa `console.log()` para {name}');
    });

    it('should return text unchanged when map is empty', () => {
      const map = new Map<string, string>();
      const result = TranslationService.restorePlaceholders('Hello world', map);

      expect(result).toBe('Hello world');
    });

    it('should handle text with no matching placeholders', () => {
      const map = new Map<string, string>();
      map.set('__CODE_99__', '`unused`');
