...
const mockedIsTextBasedFile = isTextBasedFile as jest.MockedFunction<
  typeof isTextBasedFile
>;
const mockedIsStructuredFile = isStructuredFile as jest.MockedFunction<
  typeof isStructuredFile
>;
const mockedGetFileSize = getFileSize as jest.MockedFunction<
  typeof getFileSize
>;
const mockedSafeReadFileSync = jest.mocked(
  safeReadFileSync
...
describe('FileTranslationHandler', () => {
  ...
  beforeEach(() => {
    ...
    mockDocHandler = {
      translateDocument: jest
        .fn()
        .mockResolvedValue('Translated doc.pdf -> /tmp/output.pdf'),
      ctx: result.ctx,
    };
  ...
  describe('translateFile()', () => {
    ...
    it('should throw ValidationError for multi-target with stdout output', async () => {
      await expect(
        handler.translateFile(
          '/tmp/file.txt',
          defaultOptions({ to: 'de,fr', output: '-' })
        )
      ).rejects.toThrow(ValidationError);
      await expect(
        handler.translateFile(
    ...
    it('should call fileTranslationService.translateFileToMultiple for multi-target', async () => {
      mocks.fileTranslationService.translateFileToMultiple.mockResolvedValue([
        { targetLang: 'de', text: 'Hallo', outputPath: '/tmp/out/file.de.txt' },
        {
          targetLang: 'fr',
          text: 'Bonjour',
          outputPath: '/tmp/out/file.fr.txt',
        },
      ]);

      const result = await handler.translateFile(
        '/tmp/file.txt',
        defaultOptions({ to: 'de,fr', output: '/tmp/out' })
      );

      expect(
        mocks.fileTranslationService.translateFileToMultiple
      ).toHaveBeenCalled();
    ...
    describe('multi-target glossary + translation memory', () => {
        ...
        mocks.fileTranslationService.translateFileToMultiple.mockResolvedValue([
          {
            targetLang: 'de',
            text: 'Hallo',
            outputPath: '/tmp/out/file.de.txt',
          },
          {
            targetLang: 'fr',
            text: 'Bonjour',
      ...
      it('throws ValidationError for --glossary without --from in multi-target', async () => {
        ...
        expect((err as Error).message).toContain(
          'Source language (--from) is required'
        );
      });
      ...
      it('resolves glossary name to ID and passes it to translateFileToMultiple', async () => {
        mocks.glossaryService.resolveGlossaryId.mockResolvedValue(
          'glossary-abc-123'
        );
        ...
        expect(mocks.glossaryService.resolveGlossaryId).toHaveBeenCalledTimes(
          1
        );
        expect(mocks.glossaryService.resolveGlossaryId).toHaveBeenCalledWith(
          'my-glossary',
          { from: 'en', targets: ['de', 'fr'] }
        );
        const call =
          mocks.fileTranslationService.translateFileToMultiple.mock.calls[0]!;
        expect(call[2]).toEqual(
          expect.objectContaining({ glossaryId: 'glossary-abc-123' })
        );
      });
