...
describe('extractGlobLiteralPrefix', () => {
  describe('clean patterns', () => {
    it('extracts prefix up to the first wildcard segment', () => {
      expect(extractGlobLiteralPrefix('src/locales/**/*.json')).toBe(
        'src/locales'
      );
    });
    ...
    it('handles nested literal segments with brace at the end', () => {
      expect(extractGlobLiteralPrefix('src/app/{foo,bar}/*.json')).toBe(
        'src/app'
      );
    });

    it('returns the whole pattern when no wildcards are present', () => {
      expect(extractGlobLiteralPrefix('src/app/routes.ts')).toBe(
        'src/app/routes.ts'
      );
    });
    ...
    it('treats a segment with escaped wildcard as literal', () => {
      expect(extractGlobLiteralPrefix('src/\\*literal/**/*.ts')).toBe(
        'src/\\*literal'
      );
    });

    it('preserves an absolute-style prefix within the project root when all literal', () => {
      expect(extractGlobLiteralPrefix('src/locales/en/**.json')).toBe(
        'src/locales/en'
      );
    });
  });
  ...
  describe('path-traversal rejection', () => {
    it('rejects a leading parent-directory segment', () => {
      expect(() => extractGlobLiteralPrefix('../sensitive/**')).toThrow(
        ConfigError
      );
    });

    it('rejects a mid-pattern parent-directory segment', () => {
      expect(() => extractGlobLiteralPrefix('src/../etc/**')).toThrow(
        ConfigError
      );
    });

    it('rejects brace-expansion containing parent-directory', () => {
      expect(() => extractGlobLiteralPrefix('{..,src}/**/*.json')).toThrow(
        ConfigError
      );
    });

    it('rejects nested brace-expansion containing parent-directory', () => {
      expect(() => extractGlobLiteralPrefix('src/{app,..}/**')).toThrow(
        ConfigError
      );
    });
    ...
    it('rejects extglob alternation containing parent-directory', () => {
      expect(() => extractGlobLiteralPrefix('src/+(foo|..)/**')).toThrow(
        ConfigError
      );
    });
