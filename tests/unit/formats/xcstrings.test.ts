...
describe('XcstringsFormatParser', () => {
    ...
    it('should return entries in insertion order (consumers sort downstream)', () => {
      const content = JSON.stringify({
        ...
        strings: {
          zebra: {
            localizations: {
              en: { stringUnit: { state: 'translated', value: 'Z' } },
            },
          },
          apple: {
            localizations: {
  ...
  describe('reconstruct()', () => {
    it('should add translations for a locale', () => {
      ...
      expect(data.strings.greeting.localizations.fr.stringUnit.value).toBe(
        'Bonjour'
      );
      expect(data.strings.farewell.localizations.fr.stringUnit.value).toBe(
        'Au revoir'
      );
    });
    ...
    it('should set state to translated', () => {
      ...
      const data = JSON.parse(result);
      expect(data.strings.greeting.localizations.es.stringUnit.state).toBe(
        'translated'
      );
    });
    ...
    it('should preserve existing localizations for other locales', () => {
      ...
      const result = parser.reconstruct(SAMPLE, entries, 'fr');
      const data = JSON.parse(result);
      expect(data.strings.greeting.localizations.en.stringUnit.value).toBe(
        'Hello'
      );
      expect(data.strings.greeting.localizations.de.stringUnit.value).toBe(
        'Hallo'
      );
      expect(data.strings.greeting.localizations.fr.stringUnit.value).toBe(
        'Bonjour'
    ...
    it('should create localizations structure when absent', () => {
      ...
      const data = JSON.parse(result);
      expect(data.strings.new_key.localizations.de.stringUnit.value).toBe(
        'Neu'
      );
    });
  ...
  describe('round-trip', () => {
    it('should extract translations after reconstruct', () => {
      ...
      expect(extracted.find((e) => e.key === 'greeting')!.value).toBe(
        'Bonjour'
      );
      expect(extracted.find((e) => e.key === 'farewell')!.value).toBe(
        'Au revoir'
      );
    });
    ...
    it('should chain reconstructs for multiple locales', () => {
      ...
      expect(data.strings.greeting.localizations.es.stringUnit.value).toBe(
        'Hola'
      );
      expect(data.strings.greeting.localizations.fr.stringUnit.value).toBe(
        'Bonjour'
