...
describe('translation-validator', () => {
  describe('validateTranslation', () => {
    ...
    describe('ICU-aware placeholders', () => {
      it('should not report plural branch bodies as placeholders', () => {
        ...
        const phIssues = result.issues.filter(
          (i) => i.check === 'placeholders'
        );
      ...
      it('should still error when a placeholder inside a branch is dropped', () => {
        ...
        const translation =
          '{count, plural, one {hat # Artikel} other {hat # Artikel}}';
        const result = validateTranslation('key', source, translation);
      ...
      it('should pass when placeholders inside branches are preserved', () => {
        const source =
          '{count, plural, one {{name} has # item} other {{name} has # items}}';
        const translation =
          '{count, plural, one {{name} hat # Artikel} other {{name} hat # Artikel}}';
        const result = validateTranslation('key', source, translation);
        const phIssues = result.issues.filter(
          (i) => i.check === 'placeholders'
        );
      ...
      it('should warn when a branch gains an extra placeholder', () => {
        const source = '{count, plural, one {# item} other {# items}}';
        const translation =
          '{count, plural, one {{extra} Artikel} other {Artikel}}';
        const result = validateTranslation('key', source, translation);
    ...
    describe('ICU brackets', () => {
      it('should pass when ICU brackets are balanced', () => {
        ...
        const result = validateTranslation('key', source, translation);
        expect(
          result.issues.filter((i) => i.check === 'icu-brackets')
        ).toHaveLength(0);
      });
      ...
      it('should error when ICU closing bracket is missing', () => {
        ...
        const translation = '{count, plural, one {# Artikel} other {# Artikel}';
        const result = validateTranslation('key', source, translation);
        const icuIssues = result.issues.filter(
          (i) => i.check === 'icu-brackets'
        );
      ...
      it('should handle nested ICU select/plural correctly', () => {
        const source =
          '{gender, select, male {{count, plural, one {He has # item} other {He has # items}}} female {{count, plural, one {She has # item} other {She has # items}}} other {They have # items}}';
        const translation =
          '{gender, select, male {{count, plural, one {Er hat # Artikel} other {Er hat # Artikel}}} female {{count, plural, one {Sie hat # Artikel} other {Sie hat # Artikel}}} other {Sie haben # Artikel}}';
        const result = validateTranslation('key', source, translation);
        const icuIssues = result.issues.filter(
          (i) => i.check === 'icu-brackets'
        );
  ...
  describe('length ratio', () => {
    it('should warn when translation exceeds 150% of source length', () => {
      ...
      const translation =
        'This is a very much longer translation that far exceeds the original';
      const result = validateTranslation('key', source, translation);
