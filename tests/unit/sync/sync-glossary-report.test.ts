    ...
    const lock = makeLockFile({
      'en/common.json': {
        greeting: {
          ...
          translations: { de: translated('de-v1') },
        },
      },
      'en/admin.json': {
        header: {
          source_hash: 'h',
          source_text: 'Dashboard',
          translations: { de: translated('de-v2') },
        },
      },
    });
  ...
  describe('unreadable targets', () => {
    it('reports a missing locale file instead of inventing an inconsistency', () => {
      const lock = makeLockFile({
        'en/common.json': {
          greeting: {
            source_hash: 'h',
            source_text: 'Hi',
            translations: { de: translated('de-hashA') },
          },
        },
        'en/other.json': {
          greeting2: {
            source_hash: 'h',
            source_text: 'Hi',
            translations: { de: translated('de-hashB') },
          },
        },
      });
      ...
      expect(report.missingTargets).toEqual([
        { filePath: 'en/other.json', locale: 'de' },
      ]);
    });
    ...
    it('never emits a lock hash as a translation string', () => {
      ...
      const allTranslations = report.inconsistencies.flatMap(
        (inc) => inc.translations
      );
      ...
      const lock = makeLockFile({
        'a.json': {
          k1: {
            source_hash: 'h',
            source_text: 'Hi',
            translations: { de: translated('x'), fr: translated('x') },
          },
          k2: {
            source_hash: 'h2',
            source_text: 'Bye',
            translations: { de: translated('y') },
          },
        },
      });
    ...
    const lock = makeLockFile({
      'en/common.json': {
        greeting: {
          source_hash: 'h1',
          source_text: 'Hello',
          translations: { de: translated('hash1') },
        },
        farewell: {
          source_hash: 'h2',
          source_text: 'Goodbye',
          translations: { de: translated('hash2') },
        },
        save: {
          source_hash: 'h3',
          source_text: 'Save',
          translations: { de: translated('hash3') },
        },
      },
    });
