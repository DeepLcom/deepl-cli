...
describe('ICU structure validation', () => {
  it('should pass an intact translation', () => {
    expect(
      severityOf(SOURCE, '{count, plural, one {# Artikel} other {# Artikel}}')
    ).toBe('pass');
  });
  ...
  it('should flag translated plural selectors', () => {
    expect(
      severityOf(SOURCE, '{count, plural, eins {# Artikel} andere {# Artikel}}')
    ).not.toBe('pass');
  });
  ...
  it('should flag a translated argument name', () => {
    ...
    expect(
      severityOf(SOURCE, '{Anzahl, plural, one {# Artikel} other {# Artikel}}')
    ).not.toBe('pass');
  });

  it('should flag the full live-API corruption', () => {
    const asReturnedByEngine =
      '{count, Plural, ein {# item} weiteres {# items}}';

    expect(severityOf(SOURCE, asReturnedByEngine)).not.toBe('pass');
  });

  it('should flag a dropped selector branch', () => {
    expect(severityOf(SOURCE, '{count, plural, one {# Artikel}}')).not.toBe(
      'pass'
    );
  });
  ...
  it('should tolerate reordered selectors', () => {
    ...
    expect(
      severityOf(SOURCE, '{count, plural, other {# Artikel} one {# Artikel}}')
    ).toBe('pass');
  });
  ...
  it('should handle a message with surrounding prose', () => {
    ...
    const good =
      'Sie haben {count, plural, one {# Artikel} other {# Artikel}} im Warenkorb.';
    const bad =
      'Sie haben {count, Plural, ein {# Artikel} weiteres {# Artikel}} im Warenkorb.';
