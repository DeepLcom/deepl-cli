...
import { detectI18nFiles, generateSyncConfig } from '../../src/sync/sync-init';

function makeTmpDir(label: string): string {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), `deepl-sync-init-integ-${label}-`)
  );
  return dir;
}
...
describe('sync init auto-detection with real fast-glob', () => {
  ...
  describe('JSON (i18next)', () => {
    it('should detect, generate config, and match source files with fast-glob', async () => {
      ...
      const matched = await fg(parsed.buckets['json']!.include, {
        cwd: tmpDir,
      });
  ...
  describe('YAML (Rails)', () => {
    it('should detect, generate config, and match source files with fast-glob', async () => {
      ...
      const matched = await fg(parsed.buckets['yaml']!.include, {
        cwd: tmpDir,
      });
  ...
  describe('Android XML', () => {
    it('should detect with target_path_pattern and match source files', async () => {
      ...
      expect(project!.targetPathPattern).toBe(
      ...
      const matched = await fg(parsed.buckets['android_xml']!.include, {
        cwd: tmpDir,
      });
  ...
  describe('iOS Strings', () => {
    it('should detect and match source files (locale in path)', async () => {
      ...
      const matched = await fg(parsed.buckets['ios_strings']!.include, {
        cwd: tmpDir,
      });
  ...
  describe('XLIFF (Angular)', () => {
    it('should detect with target_path_pattern and match source files', async () => {
      ...
      const parsed = YAML.parse(yaml) as ParsedConfig;
      expect(parsed.buckets['xliff']!.target_path_pattern).toBe(
        'src/locale/messages.{locale}.xlf'
      );

      const matched = await fg(parsed.buckets['xliff']!.include, {
        cwd: tmpDir,
      });
